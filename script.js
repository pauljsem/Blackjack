...
Deck.prototype = {
  //create a standard 52-card deck and assign names, values and suits
  ...
  } //randomize the cards -- a.k.a. shuffle
};
...
let getScore = (arr) => {
  ...
  if (aceCount > 0) {
    ...
    for (let j = 0; j <= aceCount; j++) {
      //check if the score is <=11 for each ace, and add 10 each time it returns true
      if (score <= 11) {
        score += 10;
      }
    }
...
let shuffleBox = () => {
  //show the shuffling message and shuffle the deck, then deal
  if (placedBet === true) {
    newDeck.shuffle();
    $("#shuffleDiv").show();
...
let deal = () => {
  ...
  dealer.hand.push(newDeck.cards.pop()); //push the first two cards to the dealer
  $(
    "<div>" + player.hand[0].name + " of " + player.hand[0].suit + "</div>"
  ).appendTo("#playerCards"); //show the card names and suits
  $(
    "<div>" + player.hand[1].name + " of " + player.hand[1].suit + "</div>"
  ).appendTo("#playerCards");
  $(
    "<div>" + dealer.hand[0].name + " of " + dealer.hand[0].suit + "</div>"
...
function hit () {
  ...
  $(
    "<div>" +
      player.hand[cardCount].name +
      " of " +
      player.hand[cardCount].suit +
      "</div>"
  ).appendTo("#playerCards"); //display last-drawn card
...
function stand() {
  $("#secondCard").html(
    "<div>" + dealer.hand[1].name + " of " + dealer.hand[1].suit + "</div>"
...
let dealersTurn = () => {
  ...
  while (dealer.score <= 17) {
    ...
    $(
      "<div>" +
        dealer.hand[index].name +
        " of   " +
        dealer.hand[index].suit +
        "</div>"
    ).appendTo("#dealerCards");
  }
...
let checkScore = () => {
  if (dealer.score > 17) {
    //if the dealer has more than 17
    if (dealer.score > player.score) {
      //AND if it's more than the player's score
      if (dealer.score <= 21) {
        //AND it's less than or === 21, then:
